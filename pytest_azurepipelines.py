...
def remove_main_from_each_html_report_file(reportdir):
    """
    Since <main> tags do not work inside the iframe used by Azure DevOps,
    replace the <main> tags with <div>s in each HTML file generated by pytest
    report.
    This enables a good UX when reading reports in the portal.
    """
    style_fragment = "\n<style>\n" + get_resource_file_content("style.css") + "\n</style>\n"
    ...
    for file in os.listdir(reportdir):
        if file.endswith(".html"):
            ...
            with (open(full_path, mode="rt", encoding="utf8") as f):
                new_text = (
                    f.read()
                    .replace("<main>", '<div class="pytest-azurepipelines-main">')
                    .replace("</main>", "</div>")
                )
...
def try_to_remove_main_from_each_html_report_file(reportdir):
    try:
        remove_main_from_each_html_report_file(reportdir)
    except Exception as ex:
        print(
            "##vso[task.logissue type=warning;]{0}{1}".format(
                "Failed to remove main tags in HTML coverage reports. Error: ",
                str(ex)
            )
        )
...
def pytest_sessionfinish(session, exitstatus):
    ...
    if not session.config.getoption("no_coverage_upload") and not session.config.getoption("no_docker_discovery") and session.config.pluginmanager.has_plugin("pytest_cov"):
        ...
        if os.path.exists(covpath):
            ...
            try_to_remove_main_from_each_html_report_file(reportdir)
